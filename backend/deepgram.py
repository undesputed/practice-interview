...
def build_interviewer_prompt(role: str, focus: str = "Mixed",
    ...
    return (
        ...
        f"The interview is already in progress: you have greeted the candidate and asked them "
        f"to tell you about themselves, which counts as question 1 of {n}. The candidate's "
        f"first message is their answer to question 1, so do NOT greet again, do NOT ask them "
        f"to introduce themselves again, and never say things like 'let's start' or 'let's "
        f"begin the real interview' — it has already begun. "
        f"Ask exactly {n} question{plural} total, one at a time, in strict order, keeping a "
        f"private count of which question you are on. After each answer, move directly to the "
        f"next unanswered question. Never restart, repeat, or re-ask a question you already asked. "
